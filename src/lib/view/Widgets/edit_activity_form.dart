...
class EditActivityDialog extends StatefulWidget {
  ...
  EditActivityDialog(this.exams, this.lectures, this.activities,
      this.setActivities, this.activity);

  @override
  _EditActivityDialogState createState() => _EditActivityDialogState();
}

class _EditActivityDialogState extends State<EditActivityDialog> {
  TextEditingController _dateCtrl;
  TextEditingController _startTimeCtrl;
  TextEditingController _endTimeCtrl;
  ...
  @override
  void initState(){
    date =  this.widget.activity.startingDate;
    startTime = TimeOfDay.fromDateTime(this.widget.activity.startingDate);
    endTime = TimeOfDay.fromDateTime(this.widget.activity.endingDate);
    _dateCtrl = TextEditingController(text: DateFormat('yyyy/MM/dd').format(this.widget.activity.startingDate));
    _startTimeCtrl = TextEditingController(text: DateFormat('hh:mm a').format(this.widget.activity.startingDate));
    _endTimeCtrl = TextEditingController(text: DateFormat('hh:mm a').format(this.widget.activity.endingDate));
    _nameCtrl = TextEditingController(text: this.widget.activity.name);
    _descCtrl = TextEditingController(text: this.widget.activity.description);
    color = this.widget.activity.colorLabel;
    switch(this.widget.activity.frequency){
      case Frequency.noRepetition: frequencia = 'Não repetir'; break;
      case Frequency.everyDay: frequencia = 'Todos os dias'; break;
      case Frequency.everyWeek: frequencia = 'Todas as semanas'; break;
      case Frequency.everyMonth: frequencia = 'Todos os meses'; break;
      case Frequency.everyYear: frequencia = 'Todos os anos'; break;
    };
    freq = this.widget.activity.frequency;
    super.initState();
  }
          ...
          TextButton(
              onPressed: () {
                ...
                if (checkForOverlap(activity)) {
                  ...
                  widget.activities.remove(this.widget.activity);
  ...
  bool checkForOverlap(Activity newActivity) {
    ...
    bool dateOverlap({int day = 0, int month = 0, int year = 0}) {
      ...
      for (Activity activity in this.widget.activities) {
        if(activity == this.widget.activity){
          continue;
        }
